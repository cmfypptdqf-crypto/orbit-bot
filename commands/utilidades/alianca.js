...
module.exports = {
    ...
    async executePrefix(message, args, client) {
        ...
        if (subcmd === 'criar') {
            ...
            if (db.usuarios[userId].alianca) return message.reply('❌ Você já está em uma aliança!');
            if (Object.values(db.aliancas).find(a => a.nome === nome)) return message.reply('❌ Já existe uma aliança com este nome!');
            
            const custo = 10000;
            if ((db.usuarios[userId].carteira || 0) < custo) return message.reply(`❌ Criar uma aliança custa ${custo.toLocaleString()} Orbs!`);
            ...
            await message.reply(`✅ Aliança **${nome}** criada com sucesso!`);
        }
        
        else if (subcmd === 'info') {
            const aliancaId = db.usuarios[userId]?.alianca || args[1];
            if (!aliancaId || !db.aliancas[aliancaId]) return message.reply('❌ Aliança não encontrada!');
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: '✨ Bônus', value: `+${Math.round((nivelInfo.bonus - 1) * 100)}% em tudo`, inline: true },
            ...
            if (!aliancaId) return message.reply('❌ Você não está em uma aliança!');
            
            const alianca = db.aliancas[aliancaId];
            if (alianca.dono !== userId) return message.reply('❌ Apenas o líder pode convidar!');
            if (db.usuarios[user.id]?.alianca) return message.reply('❌ Usuário já está em uma aliança!');
            
            if (!db.convitesAlianca) db.convitesAlianca = {};
            db.convitesAlianca[user.id] = { aliancaId, expires: Date.now() + 300000 };
            saveDB(db);
            
            await message.reply(`✅ Convite enviado para ${user}! Use \`bt!alianca entrar\` para aceitar.`);
        }
        
        else if (subcmd === 'entrar') {
            if (!db.convitesAlianca || !db.convitesAlianca[userId]) return message.reply('❌ Nenhum convite pendente!');
            
            const convite = db.convitesAlianca[userId];
            const alianca = db.aliancas[convite.aliancaId];
            if (!alianca) return message.reply('❌ Aliança não encontrada!');
            ...
            await message.reply(`✅ Você entrou na aliança **${alianca.nome}**!`);
        }
