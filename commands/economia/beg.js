...
module.exports = {
    ...
    async executePrefix(message, args, client) {
        ...
        if (!cooldownCheck.available) {
            return message.reply(`⏰ Aguarde mais **${cooldownCheck.formatted}** para pedir novamente!`);
        }
        
        const eventos = [
            { texto: '👽 Um alienígena ficou com pena de você', ganho: [200, 400], cor: 0x9B59B6 },
            { texto: '🚀 Um viajante espacial jogou Orbs', ganho: [150, 300], cor: 0x3498DB },
            { texto: '🛸 Uma nave dropou suprimentos', ganho: [100, 250], cor: 0x00FF00 },
            { texto: '💫 Uma estrela cadente deixou Orbs', ganho: [180, 350], cor: 0xF1C40F }
        ];
        ...
        if (eventoCosmico) eventoResultado = await processEvent(eventoCosmico, userId, db, client);
